...
# 创建记忆模块对象,记忆模块需要结合提示词模块使用，记忆模块保存的数据当作提示词现信息传送给大模型即可
if "memory" not in st.session_state:
    st.session_state.memory = ConversationBufferMemory(memory_key="history")
# 创建提示词对象
prompt = PromptTemplate.from_template("你的名字叫朴成训，你现在要扮演一个爱豆的角色，你现在要和你的女朋友进行对话，你男朋友说的话是{input}，你需要对你女朋友的话作出回应，而且只做回应，你和你女朋友的历史对话为{history}")
